 ...
 * -------------------------------------
 * Backend for the "Question?" button embedded in the SPM261 lecture
 * decks (week3-1 and on). A student clicks it on any slide, types a
 ...
 * See SPM261_26/questions/widgets.js for the frontend half of both.
 *
 * This is a SEPARATE, independent deployment from the SPM381 backend
 * (SPM381_26/questions/backend.gs). Each course has its own Apps
 * Script project, its own Sheet, and its own /exec URL.
 *
 * SETUP (one time)
 *  1. Create (or pick) a Google Sheet to hold SPM261 submissions.
...
function doGet(e){
  ...
  return json_({ ok: true, msg: 'SPM261 questions backend is live. POST a question to submit one.' });
}
